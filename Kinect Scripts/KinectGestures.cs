...
public class KinectGestures
{
    ...
    private const int rightFoot = (int)KinectWrapper.NuiSkeletonPositionIndex.FootRight;
    private const int leftFoot = (int)KinectWrapper.NuiSkeletonPositionIndex.FootLeft;
    ...
    private const int head = (int)KinectWrapper.NuiSkeletonPositionIndex.Head;
    ...
    static int i = 0;
    static int maxindex, minindex, timer, prevminin, prevmaxin = 0;
    public static float max, min, prevmax, prevmin, diff = 0;
    public static bool right,left,forward,backward,pr1,jump = false;
    public static bool run, swipeleft, swiperight = false;

    static bool pr2, dr = false;
    public static float maxi(float[] array, int i)
    {
        max = i;
        int temp = (i + 5) % 300;
        for (int j = 0; ;)
    ...
    {	
		...
		swiperight = false;
		diff = 0.0f;
        Debug.Log("Jump ki value:  "+jump);
		if (jointsTracked [leftHandIndex]&& jointsTracked[shoulderCenterIndex])
			if (jointsPos [leftHandIndex].x >= jointsPos [shoulderCenterIndex].x+0.2f)
				swiperight = true;
			if (jointsPos [leftHandIndex].x <= jointsPos [shoulderCenterIndex].x-0.2f)
				swipeleft = true;
		if (jointsTracked [rightFoot] && jointsTracked [leftFoot]) {
			if (i == 0) {
				max = jointsPos [rightFoot].y;
				min = jointsPos [rightFoot].y;
				prevmax = max;
				prevmin = min;
			}
			if (jointsPos [rightFoot].y > max) {
				maxindex = i;
				pr2 = true;
				if (pr1) {
					prevmin = min;
				...
				//min = prevmax;
				pr1 = false;
			} else if (jointsPos [rightFoot].y <= min) {
				minindex = i;
				pr1 = true;
				if (pr2) {
					prevmax = max;
			...
			i++;
			//Debug.Log(Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y));
			if (jointsPos [rightFoot].y - jointsPos [leftFoot].y > 0.1f)
				diff = Mathf.Abs (((float)(prevmax - prevmin) / (float)(maxindex - minindex)) * 100000);
			//if(diff>200) Debug.Log("Running"+diff);
			if (diff > 150.0f && jointsPos [rightFoot].y > 0.34f) {
				run = true;
				if (dr)
					timer = 300;
				dr = false;
		...
		if (jointsTracked [hipCenterIndex]) {
			if (jointsPos [hipCenterIndex].x > 0.3f)
				right = true;
			if (jointsPos [hipCenterIndex].x < -0.3f)
				left = true;
			if (jointsPos [hipCenterIndex].z < 1.6f)
				forward = true;
			if (jointsPos [hipCenterIndex].z > 2.3f)
				backward = true;

		}
		//Debug.Log ("Swiperight= "+swiperight+". Swipeleft= "+swipeleft);

		//Debug.Log (right + " " + left + " " + forward + " " + backward);
		// Debug.Log(jointsPos[hipCenterIndex]);
            ...
            case Gestures.Jump:
                switch (gestureData.state)
                {
                    ...
                    case 1:  // gesture phase 2 = complete
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            //Debug.Log(jointsPos[hipCenterIndex].y + " " + gestureData.jointPos.y);
                            bool isInPose = jointsTracked[hipCenterIndex] &&
                                (jointsPos[hipCenterIndex].y - gestureData.jointPos.y) > 0.20f &&
                                Mathf.Abs(jointsPos[hipCenterIndex].x - gestureData.jointPos.x) < 0.25f;
            ...
            /*    case Gestures.Run:
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection - phase 1
                            ...
                            if (jointsTracked[rightFoot] && jointsTracked[leftFoot] && Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y) > 0.03f &&
                                    Mathf.Abs(jointsPos[rightFoot].y - jointsPos[leftFoot].y) < 0.15f)
                            {
                                    SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:  // gesture phase 2 = complete
                            ...
                            {
                                bool isInPose = gestureData.joint == rightHandIndex ?
                                    jointsTracked[rightFoot] && jointsTracked[leftFoot] &&
                                    (jointsPos[rightFoot].y - jointsPos[leftFoot].y) < 0.05f &&
                                    Mathf.Abs(jointsPos[rightFoot].x - gestureData.jointPos.x) <= 0.1f :
                                jointsTracked[leftFoot] && jointsTracked[rightFoot] &&
                                    (jointsPos[rightFoot].y - jointsPos[leftFoot].y) < 0.05f &&
                                    Mathf.Abs(jointsPos[leftFoot].x - gestureData.jointPos.x) <= 0.1f;
